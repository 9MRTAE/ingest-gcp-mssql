"""
Flow  : ingest_gcp_mssql_erp_contract_legacy
...
Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_contract_legacy

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_contract_legacy --table <TABLE_NAME>
    python -m flows.ingest_gcp_mssql_erp_contract_legacy --table <TABLE_NAME> --backdate -3
"""
...
def ingest_gcp_mssql_erp_contract_legacy(
    TABLE_NAME: str = '',
    JOB_BACKDATE_DEPEN: str = JOB_BACKDATE,
    JOB_STARTDATE_DEPEN: str = JOB_STARTDATE,
    JOB_ENDDATE_DEPEN: str = JOB_ENDDATE,
) -> dict[str, str]:
    """Ingest ERP tables (domain: contract_legacy) → GCS Data Lake (path: gcp-ingest-mssql/).
