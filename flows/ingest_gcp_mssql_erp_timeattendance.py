"""
Flow  : ingest_gcp_mssql_erp_timeattendance
Repo  : ingest_gcp_mssql_popcorn
DB    : GCP · MSSQL · your-db-hostname · ERP (iProp)

GCS output path:
    {bucket}/gcp-storage-parquet/timeattendance/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_timeattendance

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_timeattendance --table <TABLE>
    python -m flows.ingest_gcp_mssql_erp_timeattendance --table <TABLE> --backdate -3
    python -m flows.ingest_gcp_mssql_erp_timeattendance --table <TABLE> --startdate 2024-01-01 --enddate 2024-01-31
"""
...
from registries.registry_erp import TABLE_REGISTRY
from tasks.tasks_gcp import extract, load, transform

# ── Identity ──────────────────────────────────────────────────────────────────
DOMAIN          = 'timeattendance'
DB_NAME         = 'ERP'
GCS_APPLICATION = 'timeattendance'
FLOW_NAME       = f'ingest_{PLATFORM}_{DB_TYPE}_{DB_NAME}_{GCS_APPLICATION}'


# ── Flow ──────────────────────────────────────────────────────────────────────

@flow(name=FLOW_NAME, log_prints=True)
def ingest_gcp_mssql_erp_timeattendance(
    TABLE_NAME: str = '',
    JOB_BACKDATE_DEPEN: str = JOB_BACKDATE,
    JOB_STARTDATE_DEPEN: str = JOB_STARTDATE,
    JOB_ENDDATE_DEPEN: str = JOB_ENDDATE,
) -> dict[str, str]:
    """Ingest timeattendance tables from MSSQL ERP → GCS Data Lake.
