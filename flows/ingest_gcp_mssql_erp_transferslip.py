"""
Flow  : ingest_gcp_mssql_erp_transferslip
...
Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_transferslip

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_transferslip --table <TABLE_NAME>
    python -m flows.ingest_gcp_mssql_erp_transferslip --table <TABLE_NAME> --backdate -3
"""
...
def ingest_gcp_mssql_erp_transferslip(
    TABLE_NAME: str = '',
    JOB_BACKDATE_DEPEN: str = JOB_BACKDATE,
    JOB_STARTDATE_DEPEN: str = JOB_STARTDATE,
    JOB_ENDDATE_DEPEN: str = JOB_ENDDATE,
) -> dict[str, str]:
    """Ingest ERP tables (domain: transferslip) → GCS Data Lake (path: gcp-ingest-mssql/).
