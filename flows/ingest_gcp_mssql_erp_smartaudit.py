"""
Flow  : ingest_gcp_mssql_erp_smartaudit
Repo  : ingest_gcp_mssql_popcorn
DB    : GCP · MSSQL · your-db-hostname · ERP (iProp)

GCS output path:
    {bucket}/gcp-storage-parquet/smartaudit/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_smartaudit

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_smartaudit --table <TABLE>
    python -m flows.ingest_gcp_mssql_erp_smartaudit --table <TABLE> --backdate -3
    python -m flows.ingest_gcp_mssql_erp_smartaudit --table <TABLE> --startdate 2024-01-01 --enddate 2024-01-31
"""
