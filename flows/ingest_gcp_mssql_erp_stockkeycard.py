"""
Flow  : ingest_gcp_mssql_erp_stockkeycard
Repo  : ingest_gcp_mssql_popcorn
DB    : GCP · MSSQL · your-db-hostname · ERP (iProp)

GCS output path:
    {bucket}/gcp-storage-parquet/stockkeycard/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_stockkeycard

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_stockkeycard --table <TABLE>
    python -m flows.ingest_gcp_mssql_erp_stockkeycard --table <TABLE> --backdate -3
    python -m flows.ingest_gcp_mssql_erp_stockkeycard --table <TABLE> --startdate 2024-01-01 --enddate 2024-01-31
"""
