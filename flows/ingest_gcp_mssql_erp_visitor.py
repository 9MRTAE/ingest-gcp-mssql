"""
Flow  : ingest_gcp_mssql_erp_visitor
Repo  : ingest_gcp_mssql
DB    : GCP · MSSQL · your-db-hostname · ERP (iProp)

GCS output path:
    {bucket}/gcp-storage-parquet/visitor/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_visitor

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_visitor --table <TABLE>
    python -m flows.ingest_gcp_mssql_erp_visitor --table <TABLE> --backdate -3
    python -m flows.ingest_gcp_mssql_erp_visitor --table <TABLE> --startdate 2024-01-01 --enddate 2024-01-31
"""
