"""
Flow  : ingest_gcp_mssql_erp_favoritemenu
...
Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_favoritemenu

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_favoritemenu --table <TABLE_NAME>
    python -m flows.ingest_gcp_mssql_erp_favoritemenu --table <TABLE_NAME> --backdate -3
"""
...
def ingest_gcp_mssql_erp_favoritemenu(
    TABLE_NAME: str = '',
    JOB_BACKDATE_DEPEN: str = JOB_BACKDATE,
    JOB_STARTDATE_DEPEN: str = JOB_STARTDATE,
    JOB_ENDDATE_DEPEN: str = JOB_ENDDATE,
) -> dict[str, str]:
    """Ingest ERP tables (domain: favoritemenu) → GCS Data Lake (path: gcp-ingest-mssql/).
