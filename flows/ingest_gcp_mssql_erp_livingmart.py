...
NOTE  : Source DB = ERP แต่ load ไปที่ GCS path livingmart/
        เพราะเป็น ERP tables ที่ ETL ฝั่ง livingmart ต้องการ
        ต่างจาก ingest_gcp_mssql_ecom ซึ่ง source มาจาก ECOM DB

GCS output path:
    {bucket}/gcp-storage-parquet/livingmart/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python -m flows.ingest_gcp_mssql_erp_livingmart

Repair single table:
    python -m flows.ingest_gcp_mssql_erp_livingmart --table tmPRO_livingmart
    python -m flows.ingest_gcp_mssql_erp_livingmart --table tmPRO_livingmart --backdate -3
"""
